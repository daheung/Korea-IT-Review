...
FROM employees   -- 1) employees 테이블에서 
ORDER BY salary; -- 3) salary 컬럼을 오름차순 정렬해서

-- ORDER BY에서 ASC나 DESC를 쓰지않으면 기본 오름차순(ASC) 정렬
-- 오름차순 정렬 Ascending(ASC) : 앞글자 3글자만 따서 ASC로 사용
-- 내림차순 정렬 Descending(DESC) : 앞글자 4글자 따서 DESC로 사용

-- 문자의 정렬 (first_name) asc로 정렬시 a -> z 순서대로 정렬, 
--      desc로 정렬시 z -> a 순서대로 정렬
SELECT first_name
FROM employees
ORDER BY first_name desc;

-- 날짜의 정렬(hire_date)
-- ASC 정렬시 예전 날짜부터 최신 날짜 순으로 정렬, DESC 정렬시 최신 날짜부터 과거 날짜 순으로 정렬
SELECT hire_date
FROM employees
...
SELECT EMPLOYEE_ID    -- 2) EMPLOYEE_ID 컬럼을 조회
FROM EMPLOYEES      -- 1) EMPLOYEES 테이블에서
...
SELECT DISTINCT JOB_ID -- 2) job_id 컬럼을 중복 제거하고 조회하겠다
FROM EMPLOYEES; -- 1) employees 테이블에서
-- 결과 : 19행

-- 컬럼을 여러개 넣으면 DISTICT는 어떻게 작동할까?
...
-- 별칭 붙여 조회하기
SELECT first_name AS "이름1", last_name AS "성1", employee_id AS "사원 번호1" 
FROM employees;
SELECT first_name AS 이름2, last_name AS 성2, employee_id AS "사원 번호2"
FROM employees;
...
-- 사원의 이름, 성, 급여를 조회하는데 급여 내림차순으로 조회할 모든 컬럼에 별칭 사용하여 조회하기
-- 쿼리문 작성시 순서
...
-- 쿼리문 실행 순서
--   FROM -> SELECT -> ORDER BY(별칭 사용 가능!!!!)
SELECT first_name "이름", last_name AS 성, salary 급여 
FROM employees 
ORDER BY 급여 DESC; 